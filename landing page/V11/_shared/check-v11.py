# -*- coding: utf-8 -*-
"""
check-v11.py - enforces the contract written at the top of _shared/v11.css.

Run from the V11_new folder:      python _shared/check-v11.py
...
WHAT IT CATCHES, and why each rule exists:
  ...
  C. A page block sets a COMPONENT property on a class-only selector.
     ...
     component, which means they belong to _shared/v11.css.
...
def main():
    if not os.path.exists(SHARED):
        print('FAIL  _shared/v11.css is missing')
        return 1
    ...
    for fn in pages:
        ...
        if 'href="_shared/v11.css"' not in html:
            issues.append('A  does not link _shared/v11.css')
        else:
            # B - it must be the last stylesheet
            tail = html.split('href="_shared/v11.css"')[-1]
        ...
        if fn != HOME:
            ...
            if 'src="_shared/v11-sub.js"' not in html:
                issues.append('A  does not load _shared/v11-sub.js')
    ...
    if problems:
        ...
        print('Move the rule into _shared/v11.css, or key the page exception on')
