...
> **Folder moved 2026-09-04:** `Prototype/landing page/` is now
> `Prototype/website/Intelligent Investing/`. `website/` is the container for site work; the brand
> folder sits inside it. `.claude/launch.json` (`ii-landing`, port 8123) and `.claude/serve-landing.py`
> were repointed. Older docs under `Doctrine/` still say "landing page" - those are prose, not paths.
...
## WHERE THIS IS, 2026-09-03 (read this first)

**Desktop is closed. The mobile pass is done except for one open bug.**

Nothing is half-applied. Every change below is in all three pages unless it says otherwise, was
verified in a served browser, and is written up with its numbers in `SESSION-2026-09-02.md`
(scroll to the 2026-09-03 sections).

| | state |
|---|---|
| Desktop 1366-2560 | closed - grid, captions, titles, tables, figure widths |
| Mobile type tiers, 390 + 375 | three pages identical |
| Mobile vertical rhythm | closed - the `24px - Nu` compensators, 23 of them |
| Mobile section boundary | closed - was 184-235px, now 80-107 |
| Mobile tap targets | closed - two exceptions recorded and deliberate |
| Home length at 390 | 14.1 screens -> **12.9** |
| Constellation in the bottom sheet | **OPEN - see Open item 1** |

**Do first on the new machine**

1. Serve it: `python -m http.server 8123` in the `website/Intelligent Investing` folder, then
   `http://localhost:8123/V16/index.html`. Never `file://` - see rule 2 below, it silently
   invalidates every measurement.
2. Read the 2026-09-03 sections of `SESSION-2026-09-02.md`. They carry the reasoning, the measured
   before/after, and three measurement traps that cost time today.
3. `shared/TITLE-TIERS.md` has four rules added on 09-02/09-03: **GRID GUTTER**, **FIGURE HEIGHT**,
   **DATA TABLES**, **MOBILE VERTICAL RHYTHM**. They are the ones most likely to be broken by the
   next edit.

**Backups from today**, all in `_archive/`, one per change:
`*-pre-mobilegap-`, `*-pre-taptargets-`, `index-pre-loopcard-`, `self-directed-pre-constfixresize-`,
`self-directed-pre-labelcollide-`, `managed-pre-trackcards-`, `managed-pre-rowheight-`,
`managed-pre-sphereplate-` (all `-20260903` or `-20260902`).

**The content-review copy** handed to the reviewer is
`../V16-content-review-20260903/` - three self-contained .html files, 8.2MB total, images and video
embedded. Rebuild it with `shared/build_share.py` then `shared/build_selfcontained.py`. Never edit
in that folder.
...
2. **Serve it, don't open the file.** `python -m http.server` at the `website/Intelligent Investing`
   folder (see
...
1. **The closing constellation renders rings-only in the bottom sheet.** On a phone-width load,
   opening `VIEW THE RECORD DIAGRAM` often shows the year rings and the centre dial but **no rays and
   no nodes**. Everything missing is gated behind `rev`, the reveal progress; the rings, years and
   dial are not.
   Established: it is **not** the label change (the pre-change backup behaves identically), there are
   **no** uncaught JS errors, and it **can** render fully - one measured run had all 30 rays drawing
   at ~47fps. That run sat ~4s before the sheet was opened; the failing ones opened at ~2.5s.
   A first theory - `rev` elapsing while the canvas was hidden, so reset `T0` when the box first gets
   a size - was implemented, measured, **did not help, and was reverted** rather than left in.
   **This needs a real browser or a phone.** The agent harness throttles rAF to ~1fps for the
   offscreen canvas (measured 30 ray-draws/sec hidden vs 1410/sec open) and fires no event at all on
   a viewport change, so every timing reading contradicted the last. Start by logging `rev` and
   `now-T0` inside `frame()` on a mobile load.

2. ~~Memo mockup is clipped.~~ **Fixed 2026-09-04, option B (Inkyung): the teaser shows the thesis,
   the kill criteria stays behind the button.** It was worse than recorded - 372 / 333 / 167px of
   content cut at 1440 / 1512 / 1920, Kill Criteria never visible at any width, and Thesis showing
   as a 19px sliver, so the card was metadata only while the section copy promises a thesis AND
   what would prove it wrong. A ratio could not fix it: `aspect-ratio:528/630` scales with `--u`
   while the block boundaries move with the text (Thesis ended at 488 / 531 / 553). Same shape as
   TITLE-TIERS > FIGURE HEIGHT. Now `#record .dr-doc{aspect-ratio:auto}` plus
   `.dr3-blk:last-child{display:none}` - the cut is a block boundary, so it is clean at every
   width (overflow -2px at all three). The bottom fade went too: nothing is cut off any more, and
   it was dimming the last ~32px of a paragraph that is fully present. Section 679 -> 867 at 1512.
   Scoped to `#record`, so the mobile sheet clone still shows both blocks - verified.
3. ~~SD closing layout.~~ **Closed 2026-09-04: keep it as it is (Inkyung).** For the record, what
   was measured at 1512 before the decision: the home closing is one row, prose (1-6) beside the
   offer (7-12), section 724. Self-Directed stacks prose AND the offer in the left 6 columns
   (88 + 66 + 193 = 414) with the constellation at 9-12 running 565, so the figure overhangs the
   left column by 151, columns 7-9 are empty, and the section is 1001. The argument for changing
   it was that the offer is the conversion point and sits below two paragraphs with a 565px
   diagram beside it, where the home puts the same box in the right half at eye level. Decided
   against; do not re-open without her.
4. ~~Figure width, SD 5 col vs managed 4 col.~~ **Settled 2026-09-03: 4 col, columns 9-12, on all
   three pages** (memo, calibration, constellation, sphere). Inkyung called it directly.
5. **Document list differs** — SD 7 items, managed 5 (SD adds `Self-Directed Investing App Terms of
...
6. **`#benchmark` title runs past the measure.** Its ink is **9.84 col** where the canon is 9. It
...
7. **Figure pull-ups are still eyeballed** — `-250u`, `-271u`, `-296u`, `-255u`, each set by hand to
...
8. ~~Consolidate `--prose` and `--prose-cols` into one spelling.~~ **Done 2026-09-04.** They were
   two designs, not two spellings: `--prose` a hardcoded length (807.5u / 947.08u), `--prose-cols`
   a count fed through an inline formula that used `--gutter` (clamp(16px,1.5vw,24px)) rather than
   the rail's 30u. Now one source - the count - with `--prose` derived from the rail:
   `calc((1645 * var(--u) + 30 * var(--u)) / 12 * var(--prose-cols) - 30 * var(--u))`, which is
   807.5u at 6 and 947.08u at 7, the same numbers with no magic constant. Every call site now
   reads `var(--prose)`.
   **It must stay a LENGTH.** A first attempt expressed it as a percentage and that halved the
   measure wherever `--prose` is used inside a box narrower than the rail - a 602px group capped
   its body at 290, and managed grew 374px. Reverted and redone. Verified after: index 9,512 and
   managed 9,969 at 1920, both identical to before. Pricing / newsroom / manifesto prose went
   602 -> 609px, which is the point: they now measure the same 6 rail columns as everyone else.
9. `--t-mono12` is redundant at desktop — absorb it.
10. `.calcard` is dead CSS across 8 pages, and holds the only 0.5px hairlines.
11. `--r-app-in:6px` is declared but unused — `.dr3-*` and `.cal-*` inner boxes are all radius 0.
12. ~~Remaining one-line mono labels at 1.5 instead of 1.4.~~ **Checked 2026-09-04 - the premise was
    wrong.** Measured across the three pages, the two ladders are already consistent: the label /
    chrome tier (`.eyeb`, `.mlink span`, `.cta-fill`, `.ncta`, `.step b`, `.rows3 .row b`,
    `.fpanel .fhead span`) is 1.5 by design, the meta tier (`.figcap`, `thead th`, `.acct3 .a .lim`)
    is 1.4. `thead th` already renders 1.4 - the base ladder beats managed's own 1.5 declaration.
    `.k` differs between pages because they are different components: index `.loopfig .k` (legend,
    1.4) vs self-directed `.dr3-lv .k` (inside a product mockup, which the ladder deliberately
    excludes). What WAS wrong: the 1.4 list also named `.src` and `.metaline`, which a later rule
    sets to 1.5 and wins - the list described the opposite of what shipped. Removed from the list
    on all three pages; the scoped `.gap2 .g2stat .src` / `.stats3 .src` / `.dstats .src` stay,
    they are more specific and still win at 1.4. Zero render change, verified.
13. `.menuov-cta` is the only mono at weight 500 (5 pages) — intent unconfirmed.
14. ~~Mobile `.metaline` / `.figcap` declarations are tangled.~~ **Done 2026-09-03.** The running-note
    tier is mono11/1.5 on both pages, the 6-col cap is in the mobile reset, and the mobile pass ran:
    tiers, vertical rhythm, section boundaries and tap targets are all closed. See the state table at
    the top of this file.
15. ~~The `.constfig` move script does not restore the figure when a narrow window is widened.~~
    **Fixed 2026-09-03.** Both directions were broken, not just the reported one. The sheet and its
    button are now built at every width and only the node's PLACE follows the breakpoint; it listens
    to `mq.change` **and** `window.resize`, because some environments fire neither reliably. Where it
    still renders rings-only is Open item 1, a different bug.
16. **Compliance / content, from `SESSION-2026-08-31.md` §8:** managed has 9 placeholders; the home's
